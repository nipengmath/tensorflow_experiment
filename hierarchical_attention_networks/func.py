...
class ptr_net:
    ...
    def __call__(self, init, match, d, mask):
        with tf.variable_scope(self.scope):
            ...
            u = pointer(d_match, init * self.dropout_mask, d, mask)
            # # [batch, c_maxlen, hidden * 2 * 2]
            return u
...
def bi_lstm_unit(inputs, name_scope, hidden_size, reuse_flag=False):
    with tf.variable_scope("bi_lstm_" + str(name_scope), reuse=reuse_flag):
        ...
        outputs, hidden_states = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell,
                                                                 lstm_bw_cell,
                                                                 inputs,
                                                                 dtype=tf.float32)
        feature = tf.concat([outputs[0], outputs[1]], axis=-1)
        return feature


def bi_shortcut_stacked_lstm_return_sequences(inputs,
                                              name_scope,
                                              hidden_size_list,
                                              dropout_keep_prob,
                                              reuse_flag=False):
        ...
        inputs_copy = inputs
        # layer1
        feature1 = bi_lstm_unit(inputs, str(name_scope) + "layer_1",
                                hidden_size_list[0], reuse_flag=reuse_flag)
        feature1 = tf.nn.dropout(feature1, keep_prob=dropout_keep_prob)

        # layer2
        inputs2 = tf.concat([inputs_copy, feature1], axis=-1)
        feature2 = bi_lstm_unit(inputs2, str(name_scope) + "layer_2",
                                hidden_size_list[1], reuse_flag=reuse_flag)
        feature2 = tf.nn.dropout(feature2, keep_prob=dropout_keep_prob)

        # layer3
        inputs3 = tf.concat([inputs_copy, feature1, feature2], axis=-1)
        feature = bi_lstm_unit(inputs3, str(name_scope) + "layer_3",
                               hidden_size_list[2], reuse_flag=reuse_flag)
        feature = tf.nn.dropout(feature, keep_prob=dropout_keep_prob)
        return feature
